...
K8_ENDPOINT="$(op read "op://world_site/pixie_minio_prod/k8_url")"
BUCKET="$(op read "op://world_site/pixie_minio_prod/bucket")"


# check if values are retrieved successfully
if [[ -z "$APP_KEY" || -z "$APP_SECRET"  || -z "$ENDPOINT" || -z "$K8_ENDPOINT" || -z "$BUCKET" ]]; then
  echo "Error: failed to get minio app secrets from 1Password."
  exit 1
fi
...
kubectl -n world create secret generic $SECRET_NAME \
  ...
  --from-literal=K8_ENDPOINT="$K8_ENDPOINT" \
